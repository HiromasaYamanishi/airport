...
d = {'X': {'train': X_train, 'val': X_valid, 'test': X_test},
    ...
    'orig_X': orig_X,
    'orig_X_encoded': X}
save_pkl('../data/processed_data.pkl', d)
...
save_pkl('../data/explainer_optuna_valid.pkl', explainer)
save_pkl('../data/shap_values_optuna_valid.pkl', shap_values)
with open('../data/model/best_lgbm_optuna_valid.pkl', 'wb') as f:
    pickle.dump(bst, f)
plt.figure()
shap.summary_plot(shap_values, X_test_shap) #左側の図
plt.savefig('../data/shap_summary_optuna_valid.jpg')
plt.figure()
shap.summary_plot(shap_values, X_test_shap, plot_type='bar') #右側の図
plt.savefig('../data/shap_summary_bar_optuna_valid.jpg')
...
shap.plots._waterfall.waterfall_legacy(10, 
                                shap_values[n,:], X_test_shap.iloc[n,:])
plt.savefig('../data/shap_test_optuna_0_valid.jpg') 
