...
def calc_accuracy(model):
    ...
    plt.savefig('../data/confusion_matrix_optuna_valid.jpg')
    
def save_model(model):
    with open(f'../data/model/lgbm/lgbm_optuna.pkl', 'wb') as f:
        pickle.dump(model, f)
...
with open('/home/yamanishi/project/airport/src/data/model/best_lgbm_optuna_2000epoch.pkl', 'rb') as f:
    bst = pickle.load(f)      
print('loaded model')
...
save_pkl('../data/explainer_optuna.pkl', explainer)
save_pkl('../data/shap_values_optuna.pkl', shap_values)
with open('../data/model/best_lgbm_optuna.pkl', 'wb') as f:
    pickle.dump(bst, f)

plt.figure()
shap.summary_plot(shap_values, X_test_shap) #左側の図
plt.savefig('../data/shap_summary_optuna.jpg')
plt.figure()
shap.summary_plot(shap_values, X_test_shap, plot_type='bar') #右側の図
plt.savefig('../data/shap_summary_bar_optuna.jpg')
...
shap.plots._waterfall.waterfall_legacy(10, 
                                shap_values[n,:], X_test_shap.iloc[n,:])
plt.savefig('../data/shap_test_optuna_0.jpg') 
