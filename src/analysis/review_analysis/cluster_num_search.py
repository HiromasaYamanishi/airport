    ...
    for model in ['luke', 'sentence']:
        ...
        for posneg, d in enumerate([pos_all_d, neg_all_d]):
            ...
            for i,spot in enumerate(kagawa_popular_spots):
                ...
                ds_tmp[spot] = (emb, sents)
            for div in np.arange(20, 120, 10):
                for min_cluster_num in np.arange(7, 40, 3):
                    for i,spot in enumerate(kagawa_popular_spots):
                        emb, sents = ds_tmp[spot]
                        pca=umap.UMAP(random_state=0, n_neighbors=15, min_dist=0.1)
                        emb_2d = pca.fit_transform(emb)
                        clustering = HDBSCAN(min_cluster_size=max(len(sents)//div, min_cluster_num)).fit(emb_2d)
                        cluster_num = len([i for i in np.unique(clustering.labels_) if i!=-1])
                        #print(np.unique(clustering.labels_))
                        df = pd.DataFrame({'spot': [spot], 'posneg':[posneg], 'sentence_model': [model], 'min_dist': [0.1], 'n_neighbors': [15], 'algorithm':['auto'], 'div': [div], 'min_cluster_num': [min_cluster_num],
                                           'min_cluster_size': [max(len(sents)//div, min_cluster_num)], 'max_cluster_size': [None], 'original_sentence_num': [len(sents)],'cluster_num': [cluster_num]})
                        df.to_csv('../data/parameter_search/cluster_num_search.csv', mode='a', header=False)
