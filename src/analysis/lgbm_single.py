...
def save_model(model):
    with open(f'../data/model/lgbm/lgbm_cv.pkl', 'wb') as f:
        pickle.dump(model, f)
...
if under_sample:
    target_count = {k:max(v, 40000) for k,v in zip(y_train.value_counts().index, y_train.value_counts().values)}
...
params = {
    ...
    'num_leaves': 80,  # ツリーの葉の最大数
    'learning_rate': 0.05,  # 学習率
    'feature_fraction': 0.99,  # 特徴量の使用割合
    'bagging_freq': 5,
    'max_depth': 7,
...
num_round = 5000 # 学習の反復回数
early_stop_count = 50

bst = None
bst_score = 10000
for depth in [7,9]:
    for leave in [  200, 250]:
        ...
        if model.best_score['valid_0']['multi_logloss']<bst_score:
            bst = model
            bst_score = model.best_score['valid_0']['multi_logloss']
...
print(bst.params)
calc_accuracy(bst)
save_pkl('../data/best_params_cv.pkl',bst.params)
save_pkl('../data/model/best_model_5000epoch_cv.pkl',bst)
...
save_pkl('../data/shap_values_cv.pkl', shap_values)
save_pkl('../data/explainer_cv.pkl', explainer)
plt.figure()
shap.summary_plot(shap_values, X_test_shap) #左側の図
plt.savefig('../data/shap_summary_cv.jpg')
plt.figure()
shap.summary_plot(shap_values, X_test_shap, plot_type='bar') #右側の図
plt.savefig('../data/shap_summary_bar_cv.jpg')
...
shap.plots._waterfall.waterfall_legacy(10, 
                                shap_values[n,:], X_test_shap.iloc[n,:])
plt.savefig('../data/shap_test_0.jpg') 

with open('../data/model/best_lgbm_cv.pkl', 'wb') as f:
    pickle.dump(bst, f)
