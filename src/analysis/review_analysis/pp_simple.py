...
{review}"
        ...
        for i in tqdm(range(0, len(df_general), batch_size)):
            ...
            for j in range(batch_size):
                ...
                prompts.append(prompt.format(review=df_general.loc[i+j, 'topic']))
    ...
    def refine_sentence(self, df_simple: pd.DataFrame):
        ...
        '''
        prompt = "次の文章から内容の重複や繰り返しを取り除き，日本語として簡潔で自然な文章に変えてください.また，多数の文章からなる場合は読みやすくなるように要約してください．100文字程度の文章を生成してください.次は例です\n\
例1:\n\
入力:\n\
...
if __name__=='__main__':
    ...
    pp = PostProcessor()
    # df_simple = pd.read_csv('../data/topic_summary/topic_summary_incontext_kagawa_luke_all_simple.csv',
    #                  names=['index', 'spot', 'posneg', 'cluster', 'topics'])
    df_simple = pd.read_csv(args.df_topic_path,
                            names=['index', 'spot', 'posneg', 'cluster', 'topics'])
    
    with open(args.group_path, 'rb') as f:
        groups = pickle.load(f)
    # with open('../data/groups/group_topic_summary_incontext_kagawa_luke_all_general.pkl', 'rb') as f:
    #     groups = pickle.load(f)
        
    df_simple = pp.merge_group(df_simple, groups)
    #print(df_simple.head())
    df_simple = pp.refine_sentence(df_simple)
    #print(df_simple.head()) 
    df_simple.to_csv(args.save_path)         
    #df_simple.to_csv('../data/topic_summary/topic_summary_incontext_kagawa_luke_all_simple_merged.csv')      
