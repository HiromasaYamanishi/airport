...
import numpy as np
import argparse
from analysis.utils.utils import prefecture_region, season
    
def calc_accuracy(model, args, y_classes):
    ...
    cmp = ConfusionMatrixDisplay(cm, display_labels=y_classes)
    plt.title(f'f1: {round(f1, 3)} Accuracy: {round(accuracy, 3)}')
    cmp.plot(cmap=plt.cm.Blues)
    
    plt.savefig(f'../data/confusion_matrix_optuna_{args.save_name}.jpg')
...
parser.add_argument('--augment_feature', action='store_true')
...
X_ = X[['home_prefecture','home_prefecture_region', 'gender', 'age', 'num',
       'beh_time_hour', 'beh_time_month', 'beh_time_year', 'beh_time_weekday','beh_time_holiday',
       'pas_time_month', 'pas_time_weekday', 'diff_time', 'pas_time_holiday','season',
       'ap21', 'ap22', 'pas_type', ]]
...
cat_cols = ['home_prefecture','home_prefecture_region', 'gender', 'age',
             'beh_time_month', 'beh_time_year', 'beh_time_weekday','beh_time_holiday',
       'pas_time_month', 'pas_time_weekday', 'pas_time_holiday','season',
       'ap21', 'ap22', 'pas_type']
...
if args.augment_feature:
    spot_feature_df = pd.read_csv('../data/feature/spot_feature.csv')
    food_feature_df = pd.read_csv('../data/feature/food_feature.csv')
    train_spot_feature = pd.DataFrame(data=np.repeat(spot_feature_df.values, len(X_train), axis=0), columns=spot_feature_df.columns)
    valid_spot_feature = pd.DataFrame(data=np.repeat(spot_feature_df.values, len(X_valid), axis=0), columns=spot_feature_df.columns)
    test_spot_feature = pd.DataFrame(data=np.repeat(spot_feature_df.values, len(X_test), axis=0), columns=spot_feature_df.columns)
    train_food_feature = pd.DataFrame(data=np.repeat(food_feature_df.values, len(X_train), axis=0), columns=food_feature_df.columns)
    valid_food_feature = pd.DataFrame(data=np.repeat(food_feature_df.values, len(X_valid), axis=0), columns=food_feature_df.columns)
    test_food_feature = pd.DataFrame(data=np.repeat(food_feature_df.values, len(X_test), axis=0), columns=food_feature_df.columns)
    X_train = pd.concat([X_train.reset_index(), train_spot_feature.reset_index(), train_food_feature.reset_index()], axis=1)
    X_valid = pd.concat([X_valid.reset_index(), valid_spot_feature.reset_index(), valid_food_feature.reset_index()], axis=1)
    X_test = pd.concat([X_test.reset_index(), test_spot_feature.reset_index(), test_food_feature.reset_index()], axis=1)
    X_train = drop_cols(X_train)
    X_valid = drop_cols(X_valid)
...
if args.use_optuna:
    ...
    if args.target=='region':
        bst = load_pkl('../data/model/best_lgbm_optuna_morefeature.pkl')
    else:
        bst = load_pkl('../data/model/best_lgbm_optuna_city.pkl')
    params = bst.params
...
print(bst.params)
calc_accuracy(bst, args, y_class)
...
save_pkl(f'../data/explainer_optuna_{args.save_name}.pkl', explainer)
save_pkl(f'../data/shap_values_optuna_{args.save_name}.pkl', shap_values)
save_pkl(f'../data/model/best_lgbm_optuna_{args.save_name}.pkl', bst)
